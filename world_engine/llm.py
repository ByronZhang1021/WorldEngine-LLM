...
def chat_json(
    ...
    last_error = None
    raw_outputs: list[str] = []  # 记录每次重试的原始输出
    for attempt in range(_retries + 1):
        raw_text = None
        try:
            raw_text = chat(messages, model=_model, temperature=_temp, max_tokens=max_tokens, label=label, config_key=config_key)
            ...
            last_error = e
            raw_outputs.append(raw_text or "")
            log("warning", f"chat_json JSON 解析失败 (第{attempt+1}次): {e}\n原始内容: {(raw_text or '')[:200]}")
            if attempt < _retries:
                _time.sleep(1)
                continue
        except Exception as e:
            last_error = e
            raw_outputs.append(raw_text if raw_text is not None else f"[调用异常] {e}")
            log("warning", f"chat_json 调用失败 (第{attempt+1}次): {e}")
            if attempt < _retries:
                _time.sleep(1)
                continue

    # 所有重试耗尽，记录到 TurnLogger（含完整 prompt 和原始输出）
    try:
        from .utils import get_turn_logger
        tl = get_turn_logger()
        if tl:
            # 提取 prompt 文本（取最后一条 user 消息）
            prompt_text = ""
            for m in reversed(messages):
                if m.get("role") == "user":
                    prompt_text = m.get("content", "")
                    break
            tl.log_llm_failure(label or "未知", prompt_text, raw_outputs,
                               str(last_error), _retries + 1)
    except Exception:
        pass
