...
function charAdd() {
    ...
    _charData.push({
        name,
        public_base: [{ text: '名字：' + name, ttl: '永久', created: now }],
        private_base: [],
        public_dynamic: [],
        private_dynamic: [],
    });
...
async function saveAllChars() {
    ...
    _deletedChars.forEach(name => {
        tasks.push(api(`/api/character/${encodeURIComponent(name)}`, { method: 'DELETE' }));
        // 同时从 STATE 中移除
        if (STATE.characters) delete STATE.characters[name];
    });
    _deletedChars = [];
    // 处理重命名：删除旧名字的文件（新名字的文件会在下面的保存中被创建）
    _renamedChars.forEach(({ oldName }) => {
        tasks.push(api(`/api/character/${encodeURIComponent(oldName)}`, { method: 'DELETE' }));
    });
    _renamedChars = [];
    // 保存所有角色数据
    _charData.forEach(c => {
        ['public_base', 'private_base', 'public_dynamic', 'private_dynamic'].forEach(field => {
            tasks.push(api(`/api/character/${encodeURIComponent(c.name)}/${field}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ entries: c[field] || [] })
            }));
        });
    });
    // 保存 state.json（含角色状态）
    tasks.push(api('/api/state', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
