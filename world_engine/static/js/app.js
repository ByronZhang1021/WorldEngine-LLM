...
async function globalSave() {
    try {
        ...
        // Settings 是全局配置，始终直接保存到 config.json
        if (tab === 'settings') {
            await saveSettings();
        } else if (_archiveMode === 'archive' && _currentArchiveId) {
            await archiveSaveEdits();
        } else {
            if (tab === 'map') await mapSave();
...
function switchTab(tabName) {
    ...
    if (tabName === 'settings') {
        ...
        if (archiveBar) archiveBar.style.display = (_archiveMode === 'archive') ? 'flex' : 'none';
    }
