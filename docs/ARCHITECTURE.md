...
| Embedding | **302.ai Embedding API**（Qwen3-Embedding-8B） | 将文本转为高维向量，用于记忆语义检索（hybrid search）。每角色独立缓存文件避免重复调用 |
| Rerank | **302.ai Rerank API**（Qwen3-Reranker-8B） | 对 embedding 粗排结果进行精排，提升记忆检索准确性 |
...
| 数据存储 | **纯文件系统（JSON）** | 无数据库，所有数据以 JSON 文件存储，便于存档/恢复/版本对比/人工编辑 |
...
├── memory.py                    ← 记忆存储（CRUD、TTL 过期清理、LLM 驱动合并、embedding 缓存管理）
├── memory_pipeline.py           ← 记忆分析管道（对话后异步分析 → 记忆操作 + 情绪更新 + 活动变更）
├── events.py                    ← 预定事件系统（CRUD + 活动链注入 + 爽约检测 + 过期清理）
├── memory_retrieval.py          ← 记忆智能检索（hybrid search: embedding + 关键词 + rerank，分层检索）
├── embedding.py                 ← 嵌入向量（API 调用 + 余弦相似度 + 费用记录）
...
│                                   ├─ models: primary_story, secondary_story, analysis, analysis_reasoning, embedding, image
│                                   ├─ telegram: bot_token, admin_bot_token, owner_id
│                                   ├─ typing_delay: 打字延迟参数
│                                   ├─ world: start_time, walk_speed, walking_speed_kmh, session_compress_threshold
│                                   ├─ memory_retrieval: rerank_model, recall_top_k, final_top_k, 权重, threshold, min_score
...
│   ├── characters/              ← 角色数据
│   │   ├── 角色名.json          ← 四个 section 的 entries 数组
│   │   └── .角色名.emb_cache.json ← embedding 缓存（隐藏文件，独立于角色数据）
...
每个角色一个 `.json` 文件，四个 section，每个 section 是 entries 数组：

```json
{
  "public_base":    [{"content": "...", "ttl": "永久", "created": ""}],
  "private_base":   [{"content": "...", "ttl": "永久", "created": ""}],
  "public_dynamic": [{"content": "...", "ttl": "24h",  "created": "2026-01-01T08:00"}],
  "private_dynamic": [{"content": "...", "ttl": "永久", "created": "2026-01-01T09:30"}]
}
```
...
### 记忆系统
- **存储**：角色 JSON 的 dynamic sections，支持 add/update/forget，带 TTL 过期（`Nh`/`Nm`/`永久`）和 LLM 驱动合并（`memory_add_count` 达到阈值自动触发）
- **分析**：每轮对话后异步触发记忆管道（`memory_pipeline.py`），LLM 分析对话内容决定记忆操作（add/update/forget）、情绪更新、活动变更标记（`activity_changed`）
- **检索**：对话生成时使用分层检索机制：
  - 记忆总数 ≤ threshold → 全量注入
  - 记忆总数 > threshold → Tier 1（最近 24 世界小时强制纳入）→ hybrid search（embedding × 0.7 + 关键词 × 0.3）→ rerank 精排 → 按 `final_top_k` 截断
- **缓存**：每角色独立 embedding 缓存文件（`.角色名.emb_cache.json`），避免重复 API 调用
...
1. **纯文件系统存储**：JSON 文件，无数据库，便于存档/恢复/调试
...
6. **Embedding 缓存**：每角色独立隐藏缓存文件（`.角色名.emb_cache.json`），避免重复 API 调用
7. **Turn 日志**：HTML 格式完整记录每轮所有 LLM 调用和费用，便于调试回溯
